...
static void loadExtensions(void)
{
    ...
    _glfw.wgl.loadedExtensions = GLFW_TRUE;
}
...
int _glfwInitContextAPI(void)
{
    ...
    _glfw.wgl.instance = LoadLibraryW(L"opengl32.dll");
                       ...
                       const _GLFWfbconfig* fbconfig)
{
    ...
    {
        window->wgl.context = _glfw.wgl.CreateContext(window->wgl.dc);
        if (!window->wgl.context)
        {
            _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                            "WGL: Failed to create OpenGL context");
        ...
        if (share)
        {
            if (!_glfw.wgl.ShareLists(share, window->wgl.context))
            {
                _glfwInputError(GLFW_PLATFORM_ERROR,
...
void _glfwDestroyContext(_GLFWwindow* window)
{
    if (window->wgl.context)
    {
        _glfw.wgl.DeleteContext(window->wgl.context);
...
//
int _glfwAnalyzeContext(const _GLFWctxconfig* ctxconfig,
                        const _GLFWfbconfig* fbconfig)
{
    GLFWbool required = GLFW_FALSE;

    if (_glfw.wgl.loadedExtensions)
...
{
    if (window)
        _glfw.wgl.MakeCurrent(window->wgl.dc, window->wgl.context);
    else
...
int _glfwPlatformExtensionSupported(const char* extension)
{
    ...
    if (_glfw.wgl.GetExtensionsStringEXT != NULL)
    {
